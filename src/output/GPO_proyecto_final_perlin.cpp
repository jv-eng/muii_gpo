...
const char* prac = "Proyecto - Perling (GpO)";   // Nombre de la practica (aparecera en el titulo de la ventana).
...
const char* vertex_prog = GLSL(
	...
	void main()
	{
		...
		color = vec3(pow(t,2), 4*t*(1-t), pow((1-t),2)); //rojo - verde - azul
...
const char* fragment_prog = GLSL(
	...
	{	
		...
		s = int(gl_FragCoord.x) + 800*int(gl_FragCoord.y);
		//s = int(normal.x) + int(800*normal.y);
		r = hash(s);
		tmp = 0.8*h + 2*normal.x;
		if (tmp < r) {
			gl_FragColor = vec4(vec3(0,0,1),1);
		} else if (tmp > r) {
			gl_FragColor = vec4(vec3(1,1,1),1);
		} else {
			gl_FragColor = vec4(vec3(0.7,0.4,0.2),1);
		}
	}
);
...
vec3 vert[N][M]; float alturas[N][M];
...
objeto crear_malla(void)
{
	...
	//obtener alturas
	FILE * fid; fopen_s(&fid, "./data/perlin.dat","rb");
	if (fid != NULL) {
		unsigned int count = fread((void*) alturas, sizeof(float), N*M, fid);
		printf("%d alturas leidas del fichero\n",count);
		fclose(fid);
	} else printf("FID NULL: no existe fichero perlin.dat\n");
...
void init_scene()  
{
  ...
  glEnable(GL_CULL_FACE); glFrontFace(GL_CW); glEnable(GL_DEPTH_TEST);
...
void render_scene()
{
	...
	Model = mat4(1.0) * scale(vec3(2,2,1));  // Matriz identidad (no movemos malla)
