...
void init_scene()
{
	...
	obj = cargar_modelo("./data/esfera_256.bix");
	...
	tex0 = cargar_textura("./data/tierra.jpg", GL_TEXTURE0);
	tex1 = cargar_textura("./data/luna.jpg", GL_TEXTURE1);
	tex2 = cargar_textura("./data/marte.jpg", GL_TEXTURE2);
	tex3 = cargar_textura("./data/Jupiter.jpg", GL_TEXTURE3);

	glEnable(GL_CULL_FACE);
}

int planeta = 0;
static const GLfloat esc[] = {1.0f, 0.27f, 0.53f, 11.0f};
...
void render_scene()
{
	...
	S = glm::scale(glm::vec3(esc[planeta], esc[planeta], esc[planeta]));
	transfer_mat4("MVP", PP*VV*M*S); 
	...
	switch (key)
	{
	  ...
	  case GLFW_KEY_F1:
	  	if (action) {
			actual_texture++;
			planeta++;
			if (actual_texture > 3) {actual_texture = 0; planeta = 0;}
	  		transfer_int("unit", actual_texture);
		}
		break;
	}
